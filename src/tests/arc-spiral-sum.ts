...
const CIRC_COUNT = 100
const cir = new InstancedMesh(circle_geometry, new MeshBasicMaterial({ color: 0xffffff }), CIRC_COUNT)

const ARC_COUNT = 1
const arcs = new Group()
let arc_index_count: number
for (let i = 0; i < ARC_COUNT; i++) {
  const arc = new Mesh(new RingGeometry(1, 1.01, 64, 1), new MeshBasicMaterial({ color: 0xee6677 }))
  arc_index_count = arc.geometry.index.count
  arcs.add(arc)
}
...
const updateScene = (screenVector: Vector2) => {
  ...
  for (let n = 0, l = sum.count; n < l; n++) {
    ...
    if (n % 2 == 1) {
      cir.setMatrixAt((n - 1) / 2, _m2)

      col.lerpColors(c1, c2, n / 10 % 1)
      cir.setColorAt((n - 1) / 2, col)
    }
    ...
    arcs.position.set(0, 0, 20)
    if ((n - 1) / 2 < ARC_COUNT && n % 2 === 1) {
      arc_angle = Math.atan2(x / n, 1)
      arc_percentage = arc_angle > 0 ? arc_angle / Math.PI / 2 : 1 + arc_angle / Math.PI / 2
      arc_scale = previous_position.length()

      const arc = arcs.children[(n - 1) / 2] as Mesh
      arc.geometry.index.count = Math.floor(arc_index_count * arc_percentage / 6) * 6
      arc.scale.setScalar(arc_scale * RADIUS)
      //arc.position.copy(_v3)
    }
...
const arc_spiral_sum_scene = scene
export { arc_spiral_sum_scene }

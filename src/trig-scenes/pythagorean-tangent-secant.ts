...
scene.userData.title = '1<sup>2</sup> + tan<sup>2</sup>θ = sec<sup>2</sup>θ'
...
const updateScene = (screenVector: Vector2) => {
  ...
  ex.position.set(Math.sign(cosAngle) * RADIUS, 0, 0)
  sin.position.set(0, RADIUS * sinAngle, 0)
  cos.position.set(RADIUS * cosAngle, 0, 0)
  tan.position.set(RADIUS * Math.sign(cosAngle), Math.sign(cosAngle) * RADIUS * tanAngle / 2, 0)
  sec.position.set(RADIUS * Math.sign(cosAngle) / 2, RADIUS * sinAngle / cosAngle / 2 * Math.sign(cosAngle), 0)


  radius.rotation.z = cosAngle > 0 ? -Math.PI / 2 : Math.PI / 2
...
const pyt_tan_sec_scene = scene
export { pyt_tan_sec_scene }

...
function setup(){
  ...
  if (windowWidth < windowHeight){
    DIAMETER = windowWidth/3;
  } else {
    DIAMETER = windowHeight/3;
  }
  RADIUS = DIAMETER/2;
}
...
function draw(){
  if (focused){
    ...
    circle(RADIUS*cos(angle), RADIUS*sin(angle), DIAMETER);
    ...
    if (showComponents) {
      ...
      strokeWeight(2);
      line(RADIUS*cos(angle), RADIUS*sin(angle), RADIUS+RADIUS*cos(angle), RADIUS*sin(angle));
    }
    pop();
    ...
    fill(0);
    stroke(0);
    circle(RADIUS + RADIUS*cos(angle), RADIUS*sin(angle), 8);
    pop();
    ...
    text("Functions:" + "\n" + 
      "exp(j\u03B8) + 1 = cos(\u03B8) + j*sin(\u03B8) + 1" + "\n" + 
      "exp(jπ) + 1 = cos(π) + j*sin(π) + 1 " + "\n" + 
      "exp(jπ) + 1 =  -1    +   j0     + 1 " + "\n" + 
      "exp(jπ) + 1 =   0" + "\n" + 
